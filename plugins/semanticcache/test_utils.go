...
import (
	...
	mocker "github.com/capsohq/bifrost/plugins/mocker"
...
// SharedTestNamespace is the single vector-store namespace all parallel tests share.
...
// production default so test runs can't collide with a real cache. The process
// suffix prevents stale data from an interrupted prior run from producing hits.
var SharedTestNamespace = "BifrostSemanticCachePluginTest_" + uuid.NewString()[:8]
