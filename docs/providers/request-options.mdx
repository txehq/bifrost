...
| `BifrostContextKeyPromptCacheAutoInject`   | `x-bf-prompt-cache-auto-inject`                      | `bool`                | Per-request override of the provider's `prompt_cache.auto_inject`; only honored when the provider has a `prompt_cache` block configured |
...
**Context Key:** `BifrostContextKeyPromptCacheAutoInject`
**Header:** `x-bf-prompt-cache-auto-inject`
**Type:** `bool` (header values: `"true"` or `"false"`)
**Required:** No

Flip the provider's `prompt_cache.auto_inject` setting for this request only. Send `"true"` to inject a cache breakpoint on a request to a provider that has it off, or `"false"` to leave a request untouched when the provider has it on. The model capability gate still applies, so a marker is never forced onto a model that cannot use one.

<Note>
The override only takes effect on a provider that already has a `prompt_cache` block configured. A provider with no block has expressed no opinion, and a request header cannot spend a cache marker on the operator's behalf. Configure `"prompt_cache": {"auto_inject": false}` on the provider if you want callers to opt in per request. Only `auto_inject` is overridable; the TTL and injection points stay a config-level decision. See [Prompt caching](/features/prompt-caching).
</Note>
...
```bash
curl --location 'http://localhost:8080/v1/chat/completions' \
--header 'x-bf-prompt-cache-auto-inject: true' \
--header 'Content-Type: application/json' \
--data '{
...
```go
ctx := context.Background()
ctx = context.WithValue(ctx, schemas.BifrostContextKeyPromptCacheAutoInject, true)
