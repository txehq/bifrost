...
<AccordionGroup>
  ...
  <Accordion title="Compliance & Data Residency">
    ...
    {
      "provider_configs": [
        {"provider": "azure", "allowed_models": ["gpt-4o"], "key_ids": ["*"]},
        {"provider": "bedrock", "allowed_models": ["claude-3-sonnet-20240229"], "key_ids": ["*"]}
      ]
    }
    ```
  </Accordion>
...
**`key_ids: ["*"]`**: Allows every configured API key for that provider. An empty or omitted `key_ids` blocks all provider keys, so every runnable provider-config example above opts in explicitly.
...
// Request complexity (available only when classification succeeds)
complexity_tier            // SIMPLE, MEDIUM, or COMPLEX; otherwise unavailable and complexity-dependent rules fall through
```
