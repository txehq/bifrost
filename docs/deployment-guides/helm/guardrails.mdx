...
Rules are CEL expressions that fire when their condition is met. Set `target` to `"llm"` or `"mcp"` to select the request boundary the rule guards. The target defaults to `"llm"` when omitted.
