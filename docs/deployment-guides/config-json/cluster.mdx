...
| `consul_address` | string | Consul agent address (default: `localhost:8500`) |
...
<Note>
`consul_address` is the only Consul field in `config.json`. ACL tokens, TLS, namespace, and datacenter are **not** `config.json` fields - set them with Consul's standard environment variables on the Bifrost process instead. See [Consul Discovery &rarr; Authentication and TLS](/enterprise/clustering#consul-authentication-and-tls) for the full list.
</Note>
