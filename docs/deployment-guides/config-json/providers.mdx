...
Databricks keys carry a workspace URL and, optionally, an OAuth machine-to-machine service principal. Both Databricks inference surfaces are served by this one provider; `api_format` selects between them, and `auto` picks by model name.
...
<Tab title="OAuth M2M (Service Principal)">

Leave `value` unset. Bifrost mints and refreshes tokens from the workspace OIDC endpoint.
...
See [Databricks](/providers/supported-providers/databricks) for the full field reference.
