...
|-------------------|------------|-----------------|
| Alpine containers | musl | `golang:1.27.0-alpine3.24` |
| Debian/Ubuntu containers | glibc | `golang:1.27.0-bookworm` |
| Ubuntu/Debian servers | glibc | `golang:1.27.0-bookworm` |
| RHEL/CentOS servers | glibc | Native build or glibc container |
| Kubernetes (Alpine) | musl | `golang:1.27.0-alpine3.24` |
| Kubernetes (Debian) | glibc | `golang:1.27.0-bookworm` |
...
See the [hello-world plugin Makefile](https://github.com/capsohq/bifrost/blob/main/examples/plugins/hello-world/Makefile) for a complete example.
...
Bifrost is built with **Go 1.27.0**. Your plugin **must** be compiled with the exact same Go version to ensure compatibility.

```bash
# Check your Go version
go version
# Should output: go version go1.27.0 ...

# If you need to install Go 1.27.0
...
require (
    github.com/capsohq/bifrost/core v1.2.38
    // Optional: Add framework for advanced features
    // github.com/capsohq/bifrost/framework v1.1.48
...
**Solution:** Use the exact same Go version (Go 1.27.0) for both:
...
go version  # Should output: go version go1.27.0
...
```dockerfile
# Build plugin
FROM golang:1.27.0-alpine3.24 AS plugin-builder
WORKDIR /plugin
...
# Build Bifrost
FROM golang:1.27.0-alpine3.24 AS bifrost-builder
# ... (bifrost build steps)
...
3. **[Example plugins](https://github.com/capsohq/bifrost/tree/main/examples/plugins)** - Study working examples

<Note>
For questions or issues with dynamic builds and plugins, visit our [GitHub Discussions](https://github.com/capsohq/bifrost/discussions) or [Discord community](https://discord.gg/exN5KAydbU).
</Note>
