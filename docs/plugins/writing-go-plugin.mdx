...
This guide walks you through creating a native Go plugin for Bifrost using our [hello-world example](https://github.com/capsohq/bifrost/tree/main/examples/plugins/hello-world) as a reference. You'll learn how to structure your plugin, implement required functions, build the shared object, and integrate it with Bifrost.

## Prerequisites

Before you start, ensure you have:

- **Go 1.27.0** installed (must match Bifrost's Go version)
...
```bash
go get github.com/capsohq/bifrost/core@latest
```
...
require github.com/capsohq/bifrost/core v1.2.38
```
...
#### `ctx.GetModelInfo(provider, model)` <sup>v1.6.7+</sup>
...
#### `ctx.CalculateCost(resp)` <sup>v1.6.7+</sup>
...
- **All shared packages** (especially `github.com/capsohq/bifrost/core`)
...
bifrost-http: go1.27.0
  dep  github.com/maximhq/bifrost/core  v1.3.54   # <-- MISMATCH!
  dep  github.com/valyala/fasthttp      v1.55.0   # <-- MISMATCH!
```

Notice that even though the Go version matches (`go1.27.0`), the **package versions** are different - this causes the error.

**Step 2: Update your plugin dependencies**

```bash
# Update to match Bifrost's core version
go get github.com/capsohq/bifrost/core@v1.3.54
go mod tidy
...
$ go version -m my-plugin.so | grep bifrost
  dep  github.com/capsohq/bifrost/core  v1.3.54  # Now matches!
```
...
- **Full Example**: [examples/plugins/hello-world](https://github.com/capsohq/bifrost/tree/main/examples/plugins/hello-world)
- **main.go**: [Plugin implementation](https://github.com/capsohq/bifrost/blob/main/examples/plugins/hello-world/main.go)
- **Makefile**: [Build configuration](https://github.com/capsohq/bifrost/blob/main/examples/plugins/hello-world/Makefile)
- **go.mod**: [Dependencies](https://github.com/capsohq/bifrost/blob/main/examples/plugins/hello-world/go.mod)

## Real-World Plugin Examples

Explore production-ready plugins in the Bifrost repository:

- **[Mocker Plugin](https://github.com/capsohq/bifrost/tree/main/plugins/mocker)** - Mock responses for testing
- **[Logging Plugin](https://github.com/capsohq/bifrost/tree/main/plugins/logging)** - Advanced request/response logging
- **[Semantic Cache Plugin](https://github.com/capsohq/bifrost/tree/main/plugins/semanticcache)** - Cache based on semantic similarity
- **[Governance Plugin](https://github.com/capsohq/bifrost/tree/main/plugins/governance)** - Rate limiting and budget controls
- **[JSON Parser Plugin](https://github.com/capsohq/bifrost/tree/main/plugins/jsonparser)** - Parse and validate JSON responses

## Frequently Asked Questions

### Do I need to rebuild my plugin when upgrading Bifrost?

**Yes, absolutely.** Plugins must be compiled against the exact same version of `github.com/capsohq/bifrost/core` that Bifrost is using. This is a fundamental requirement of Go's plugin system.
...
go get github.com/capsohq/bifrost/core@v1.3.0
go mod tidy
    ...
    steps:
      ...
      - name: Build Plugins
        run: |
          ...
          go get github.com/capsohq/bifrost/core@${{ env.BIFROST_VERSION }}
          go mod tidy
          go build -buildmode=plugin -o my-plugin.so main.go
...
plugin was built with a different version of package github.com/capsohq/bifrost/core
```
...
- **GitHub Issues**: [Report bugs or request features](https://github.com/capsohq/bifrost/issues)
