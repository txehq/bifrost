...
helm-index: ## Repackage helm chart, regenerate index.yaml digest, then remove the .tgz
	...
	if [ -f index.yaml ]; then \
		helm repo index . --url https://github.com/capsohq/bifrost/releases/download/helm-chart-v$$CHART_VERSION --merge index.yaml; \
	else \
		helm repo index . --url https://github.com/capsohq/bifrost/releases/download/helm-chart-v$$CHART_VERSION; \
	fi && \
