...
// TestGetVirtualKeys_FromMemoryUsesDB verifies the legacy from_memory flag is
// ignored so the endpoint returns the same ConfigStore-backed response shape.
func TestGetVirtualKeys_FromMemoryUsesDB(t *testing.T) {
	SetLogger(&mockLogger{})
	...
	if store.getVirtualKeysCalls != 1 {
		t.Fatalf("from_memory path called GetVirtualKeys %d times, want 1", store.getVirtualKeysCalls)
	}
...
// TestGetVirtualKeys_FromMemoryDoesNotOverrideLimit verifies pagination still
// uses the paginated ConfigStore path when the legacy from_memory flag is set.
func TestGetVirtualKeys_FromMemoryDoesNotOverrideLimit(t *testing.T) {
	SetLogger(&mockLogger{})
	...
	if store.getVirtualKeysPaginatedCalls != 1 {
		t.Fatalf("from_memory path called GetVirtualKeysPaginated %d times, want 1", store.getVirtualKeysPaginatedCalls)
	}
...
// TestGetVirtualKeys_FromMemoryUsesDBForUserFilter verifies the legacy flag cannot
// bypass DB-backed user filtering.
func TestGetVirtualKeys_FromMemoryUsesDBForUserFilter(t *testing.T) {
	SetLogger(&mockLogger{})
	...
	if store.getVirtualKeysPaginatedCalls != 1 || store.getVirtualKeysCalls != 0 {
		t.Fatalf("expected filtered DB path, got direct/paginated calls %d/%d", store.getVirtualKeysCalls, store.getVirtualKeysPaginatedCalls)
	}
}

// TestGetVirtualKeys_FromMemoryUsesDBForOtherFilters verifies customer and team
// filters also retain DB-backed semantics when the legacy flag is present.
func TestGetVirtualKeys_FromMemoryUsesDBForOtherFilters(t *testing.T) {
	SetLogger(&mockLogger{})
