...
func mergeMCPConfigOauthDriftFixture(t *testing.T, oauthID string) (*MockConfigStore, *schemas.MCPClientConfig) {
	t.Helper()
	store := NewMockConfigStore()
	store.oauthConfigsByID[oauthID] = &tables.TableOauthConfig{
		ID:           oauthID,
...
func testMemoryEndpoint(id, name string) *tables.TableWebhookEndpoint {
...
func TestWebhookEndpointMemoryStore(t *testing.T) {
	...
	config.replaceWebhookEndpoints([]tables.TableWebhookEndpoint{*testMemoryEndpoint("ep-3", "third")})
...
func TestUpdateMCPClientConfig_RejectsNegativeToolSyncInterval(t *testing.T) {
	...
	err := store.UpdateMCPClientConfig(ctx, "neg-update", &tables.TableMCPClient{
		ClientID:         "neg-update",
		Name:             "neg-update",
		ConnectionType:   "http",
		ToolSyncInterval: -60,
