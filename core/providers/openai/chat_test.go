...
func TestToOpenAIChatRequest_NormalizesReasoningEffort(t *testing.T) {
	// Keep generic OpenAI-dialect DeepSeek coverage distinct from the built-in
	// DeepSeek provider, which translates reasoning into its native thinking field.
	customDeepSeek := schemas.ModelProvider("custom-deepseek")
	schemas.RegisterKnownProvider(customDeepSeek)
	defer schemas.UnregisterKnownProvider(customDeepSeek)
