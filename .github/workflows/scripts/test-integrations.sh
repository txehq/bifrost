...
dump_server_log() {
  echo ""
  echo "----- last 200 lines of bifrost server log ($LOG_FILE) -----"
  tail -n 200 "$LOG_FILE" 2>/dev/null || echo "   (log file unreadable)"
  echo "-----------------------------------------------------------"
}
...
if ! has_any_provider_credentials; then
  echo "No provider credentials available; SDK integration suites skipped after startup smoke test."
  exit 0
fi
