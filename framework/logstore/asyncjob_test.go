...
func TestSubmitJob_PropagatesContextValues(t *testing.T) {
	...
	requestCtx.SetValue(schemas.BifrostContextKeyVirtualKey, "sk-bf-test")
	requestCtx.SetValue(schemas.BifrostContextKey("x-bf-prom-env"), "production")
	requestCtx.SetValue(schemas.BifrostContextKey("x-bf-eh-custom"), "custom-value")
	...
	job, err := executor.SubmitJob(requestCtx, 3600, operation, schemas.ChatCompletionRequest)
	require.NoError(t, err)
...
func TestSubmitJob_AsyncFlagOverridesContextValues(t *testing.T) {
	...
	requestCtx := schemas.NewBifrostContext(context.Background(), schemas.NoDeadline)
	requestCtx.SetValue(schemas.BifrostIsAsyncRequest, false)
	var capturedCtx *schemas.BifrostContext
	var done atomic.Bool
	operation := func(bgCtx *schemas.BifrostContext) (interface{}, *schemas.BifrostError) {
		capturedCtx = bgCtx
